...
- **Недостаточность данных в некоторых сегментах**: В некоторых категориях вакансий может быть недостаточно данных, чтобы модель могла точно прогнозировать зарплату. Это может быть связано с неравномерным распределением вакансий по регионам или с неадекватной представленностью определенных профессий.
  ...
  **Решение**:
  - Стратифицированная валидация: Разбить данные для валидации с учетом регионов. Например, можно проверять модели на данных Москвы/СПб отдельно от других регионов. Это позволит избежать ситуации, когда диспропорционально большое количество данных из Москвы доминирует при обучении.
...
#### Функция потерь:
- Для обучения моделей используется **средняя абсолютная ошибка (MAE)**, так как данная функция потерь менее чувствительна к выбросам в данных, чем среднеквадратичная ошибка (MSE), что особенно важно для текущей задачи с  скошенным распределением зарплат.
