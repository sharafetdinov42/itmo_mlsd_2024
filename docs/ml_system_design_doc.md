...
- **Тип задачи (Data Science)**: задача регрессии (Supervised Learning). Модель будет обучаться на исторических данных для прогнозирования зарплаты пользователя на основе различных факторов (например, регион, опыт работы, квалификация). Цель — создать точную и эффективную модель, которая будет предоставлять точные прогнозы для пользователей, основываясь на введённых данных.

- **Цель Data Science**: разработать высококачественную регрессионную модель, которая обучается на наборе данных о зарплатах, учитывая различные параметры (опыт, навыки, регион и т.д.), и будет давать точные прогнозы. Необходимо оптимизировать модель для достижения высокого качества предсказаний и минимизации ошибок.
